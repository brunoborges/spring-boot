...
=== Microsoft Azure
Microsoft Azure supports all common Cloud scenarios for deploying a Spring Boot application. You can either deploy as an uber JAR or a WAR file, and also as a Docker image. In this section we provide you with recommendations for the simplest ways of deploying each of these, but many other ways can be used following the https://docs.microsoft.com/azure/[official documentation].
